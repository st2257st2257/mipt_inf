...
def house(scr, x, y, k):  # house
    pygame.draw.polygon(scr, color[5], [[int(x * k), int((y+30)*k)], [int(((2*x + 2.9 * 40) / 2)*k), int(y*k)],
...
def sun(scr, x=85*4, y=40, w=0, h=0, k=1, r=20, n= 20):
    ...
    aa = [np.cos((2*np.pi)*(i/n)+(2*np.pi)*(1/60)) for i in range(n)]
    bb = [np.sin((2*np.pi)*(i/n)+(2*np.pi)*(1/60)) for i in range(n)]
    for i in range(20):
        d = []
...
def tree(scr, x, y, w, h, k, n):  # tree
    ...
    branch(x, y, 20, k, 0, 0)
    branch(x, y, 20, k, 5, -15) 
    branch(x, y, 20, k, 5, -25)
    branch(x, y, 20, k, -5, -15)
    branch(x, y, 20, k, -5, -25)    
...
while not done:
    ...
    sun(screen, r=30)
    # cloud    
    clouds(screen, 10*4, 30, 0, 0, 1, 0)
    clouds(screen, 60*4, 50, 0, 0, 1, 0)
    clouds(screen, 200*2, 40, 0, 0, 1.5, 0)
    clouds(screen, 130*4, 20, 0, 0, 1, 0)
    clouds(screen, 100*4, 80, 0, 0, 1, 0)
    pygame.display.flip()
